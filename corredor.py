...
def tela_ponto():
    if game_active:
        ...
        score_superf = teste_font.render(f'Pontos: {h_atual}', False, (64, 64, 64))
    ...
    if obstaculo_list:
        for obstaculo_rect in obstaculo_list:
            ...
            if obstaculo_rect.bottom == 300:
                ...
                screen.blit(mosca_superf, obstaculo_rect)

        obstaculo_list = [obstaculo for obstaculo in obstaculo_list if obstaculo_rect.x > -100]
...
def colisao(jogador, obstaculos):
    if obstaculos:
        for obstaculo_rect in obstaculos:
            if jogador.colliderect(obstaculo_rect):
                return True
    return False
...
while True:
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            sys.exit()
        if game_active:
            if event.type == pygame.MOUSEBUTTONDOWN:  # Verifica se algum botão do mouse está pressionado
                if jogador_retangulo.collidepoint(event.pos) and jogador_retangulo.bottom >= 300:
                    # a ação só aparece quando se clica em cima do personagem
    ...
    # blit significa transferência de imagem em bloco
    ...
    if game_active:
        ...
        # Jogador
        jogador_gravidade += 1
        jogador_retangulo.y += jogador_gravidade  # mapeia a gravidade através do retângulo do personagem
        if jogador_retangulo.bottom >= 300:
            jogador_retangulo.bottom = 300
        ...
        game_active = not colisao(jogador_retangulo, obstaculo_rect_list)

    else:
        screen.fill((94, 129, 162))
        screen.blit(jogador_fixo, jogador_fixo_retangulo)
        obstaculo_rect_list.clear()
        jogador_retangulo.midbottom = (80, 300)
        jogador_gravidade += 1

        ponto_msg = teste_font.render(f'Pontos: {ponto}', False, (111, 196, 169))
